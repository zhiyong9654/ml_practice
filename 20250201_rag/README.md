...
1. I got confused about the state. We can adjust state within a conditional flow function as well, because state is passed by reference. I kept on thinking that i needed to write an 'edge function' and a separate 'conditional check function' because i thought i had to return the state object if i wanted to modify the state. But this isn't true.
